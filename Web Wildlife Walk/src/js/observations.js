...
function titleCase(s) {//added
  let arr = "";
  let str = s.split(" ");
  str.forEach((elem) => (arr += elem[0].toUpperCase() + elem.slice(1) + " "));
  arr.trim();
  return arr;
}
...
function transformObservations(observations) {
  return observations.map((observation) => {
    let obj = {
      ...
      date: new Date(Date.parse(observation.created_at)),
      name: "",
      ...
      isEndangered: Boolean(observation.taxon.endangered),
...
//transformObservations(observations.results[0]);
...
function filterObservations(observations) {
  return observations.filter(function (observation) {
    if (observation.taxon && observation.taxon.default_photo){return observation};//
  });
}
